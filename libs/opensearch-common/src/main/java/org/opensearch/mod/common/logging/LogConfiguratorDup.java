...
package org.opensearch.mod.common.logging;
...
public class LogConfiguratorDup {
     ...
     */
    /* RF - environment related code should not be present here
    ...
    public static void loadLog4jPlugins() {
        PluginManager.addPackage(LogConfiguratorDup.class.getPackage().getName());
    }
...
// #RF - this method should be moved out of here
...
//            LogManager.getLogger(LogConfiguratorDup.class).warn("Some logging configurations have %marker but don't have %node_name. "
...
//    }
    ...
    private static void configureLoggerLevels(final Settings settings) {
        if (LoggersDup.LOG_DEFAULT_LEVEL_SETTING.exists(settings)) {
            final Level level = LoggersDup.LOG_DEFAULT_LEVEL_SETTING.get(settings);
            LoggersDup.setLevel(LogManager.getRootLogger(), level);
        }
        LoggersDup.LOG_LEVEL_SETTING.getAllConcreteSettings(settings)
            // do not set a log level for a logger named level (from the default log setting)
            .filter(s -> s.getKey().equals(LoggersDup.LOG_DEFAULT_LEVEL_SETTING.getKey()) == false).forEach(s -> {
            final Level level = s.get(settings);
            LoggersDup.setLevel(LogManager.getLogger(s.getKey().substring("logger.".length())), level);
        });
    }
     ...
     */
    /* #RF - this methods needs to be moved out of here
