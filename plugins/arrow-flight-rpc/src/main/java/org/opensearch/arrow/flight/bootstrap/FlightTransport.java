...
public class FlightTransport extends TcpTransport {
    ...
    private void bindServer() {
        ...
        List<TransportAddress> transportAddresses = boundAddresses.stream()
        ...
        int publishPort = Transport.resolveTransportPublishPort(SETTING_FLIGHT_PUBLISH_PORT.get(settings), transportAddresses, publishInetAddress);
        if (publishPort < 0) {
            throw new BindTransportException(
                "Failed to auto-resolve flight publish port, multiple bound addresses " + transportAddresses +
                    " with distinct ports and none matched the publish address (" + publishInetAddress + ")."
            );
        }
    ...
    protected TcpServerChannel bind(String name, InetSocketAddress address) {
        return new FlightTcpServerChannel(address);
    }
    ...
    protected TcpChannel initiateChannel(DiscoveryNode node) throws IOException {
        ...
        ClientHolder holder = flightClients.computeIfAbsent(nodeId, id -> {
            ...
            Location location = sslContextProvider != null
                ? Location.forGrpcTls(address.getAddress(), address.getPort())
                : Location.forGrpcInsecure(address.getAddress(), address.getPort());
        ...
        return new FlightTcpChannel(holder.flightClient(), node, holder.location(), false, DEFAULT_PROFILE);
    }
    ...
    private static class FlightTcpServerChannel implements TcpServerChannel {
        private final InetSocketAddress address;
        private volatile boolean isClosed;

        FlightTcpServerChannel(InetSocketAddress address) {
