...
package org.opensearch.mod.common.geo;
...
public class GeoPoint implements ToXContentFragment {
    ...
    public GeoPoint parseGeoHash(String geohash, EffectivePoint effectivePoint) {
    ...
    public static double assertZValue(final boolean ignoreZValue, double zValue) {
        if (ignoreZValue == false) {
            throw new OpenSearchParseException("Exception parsing coordinates: found Z value [{}] but [{}] "
                + "parameter is [{}]", zValue, Names.IGNORE_Z_VALUE, ignoreZValue);
        }
        return zValue;
    }
}
