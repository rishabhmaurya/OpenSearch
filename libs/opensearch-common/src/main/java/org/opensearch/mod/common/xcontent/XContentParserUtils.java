...
package org.opensearch.mod.common.xcontent;

import org.opensearch.common.xcontent.NamedXContentRegistry;
import org.opensearch.common.xcontent.XContentLocation;
import org.opensearch.common.xcontent.XContentParser;
import org.opensearch.mod.common.ParsingExceptionDup;
 ...
 * while checking for their types and throw {@link ParsingExceptionDup} if needed.
 */
public final class XContentParserUtils {
     ...
     * @throws ParsingExceptionDup if the token is not of type {@link Token#FIELD_NAME} or is not equal to the given field name
     */
    public static void ensureFieldName(XContentParser parser, Token token, String fieldName) throws IOException {
        ...
        if (currentName.equals(fieldName) == false) {
            ...
            throw new ParsingExceptionDup(parser.getTokenLocation(), String.format(Locale.ROOT, message, fieldName, currentName));
        }
    }

    /**
     * @throws ParsingExceptionDup with a "unknown field found" reason
     */
    public static void throwUnknownField(String field, XContentLocation location) {
        ...
        throw new ParsingExceptionDup(location, String.format(Locale.ROOT, message, field));
    }

    /**
     * @throws ParsingExceptionDup with a "unknown token found" reason
     */
    public static void throwUnknownToken(Token token, XContentLocation location) {
        ...
        throw new ParsingExceptionDup(location, String.format(Locale.ROOT, message, token));
    }

    /**
     * Makes sure that provided token is of the expected type
     *
     * @throws ParsingExceptionDup if the token is not equal to the expected type
     */
    ...
    private static ParsingExceptionDup parsingException(XContentParser parser, Token expected, Token actual) {
        return new ParsingExceptionDup(parser.getTokenLocation(),
     ...
     * </ul>
     *
     * @throws ParsingExceptionDup if the token is none of the allowed values
     */
     ...
     * @throws ParsingExceptionDup if the parser isn't positioned on either START_OBJECT or START_ARRAY at the beginning
     */
    public static <T> void parseTypedKeysObject(XContentParser parser, String delimiter, Class<T> objectClass, Consumer<T> consumer)
        ...
        if (Strings.hasLength(currentFieldName)) {
            ...
            throw new ParsingExceptionDup(parser.getTokenLocation(), "Failed to parse object: empty key");
        }
    }
}
