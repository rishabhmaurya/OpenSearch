...
//        }
    ...
    public void testArrowFlightProducer() throws Exception {
        ...
        for (DiscoveryNode node : getClusterState().nodes()) {
            ...
            transportService.handleStreamRequest(previousNode, QUERY_ACTION_NAME, request, TransportRequestOptions.builder().withType(TransportRequestOptions.Type.STREAM).build(), handler);
            assertBusy(() -> {
               assertNotNull(
                   "Timeout waiting for response",  resp[0]
                );
