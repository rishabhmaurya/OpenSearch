...
public final class TraceEventTransportResponseHandler<T extends TransportResponse> implements TransportResponseHandler<T> {
    ...
    public void handleResponse(T response) {
        try {
            ...
            if (validatePrerequisites(span)) {
                for (TraceEventListener traceEventListener : traceEventsService.getTraceEventListeners().values()) {
    ...
    private boolean validatePrerequisites(Span span) {
        if (traceEventsService.getTraceEventListeners() == null ||
            traceEventsService.getTraceEventListeners().isEmpty()) {
            return false;
        }
        if (span == null) {
            return false;
        }
        if (span instanceof DiagnosticSpan && !traceEventsService.isDiagnosisEnabled()) {
            return false;
        }
        if (span.hasEnded()) {
