    ...
    fn prefetch_rg(
            ...
            && should_consult_lucene(&page_ranges, rg, PEER_CONSULT_SELECTIVITY_THRESHOLD)
        {
            ...
            if let Some(bitmap_lock) = self.peer_bitmap_cache.get(&annotation_id) {
                ...
                // A peer is optional pruning: on a successful drain, AND its
                // slice into candidates; on a failed drain, leave candidates as
                // the page-pruned universe and let DataFusion's FilterExec
                // evaluate the retained predicate (safe — peers keep `original`).
                match drained {
                    Ok(full_bm) => {
                        let slice = Self::rg_slice(full_bm, rg, seg_start, min_doc, max_doc);
                        candidates &= slice;
                    }
                    Err(e) => {
                        log_debug!("[scf-rust] peer bitmap drain failed (DF will filter): {}", e);
                    }
                }
            }
        }
