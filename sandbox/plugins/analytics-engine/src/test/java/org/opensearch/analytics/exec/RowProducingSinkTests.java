...
public class RowProducingSinkTests extends OpenSearchTestCase {
    ...
    public void testFeedSingleResponse() {
        ...
        // VarChar vectors return org.apache.arrow.vector.util.Text
        assertEquals("alice", it.next()[0].toString());
        assertTrue(it.hasNext());
        assertEquals("bob", it.next()[0].toString());
    ...
    public void testFeedMultipleResponsesPreservesOrder() {
        ...
        Iterator<Object[]> it = sink.readResult().iterator();
        assertEquals("1", it.next()[0].toString());
        assertEquals("2", it.next()[0].toString());
        assertEquals("3", it.next()[0].toString());
        assertTrue(it.hasNext() == false);
