...
package org.opensearch.mod.rest;
...
public enum RestStatus {
    ...
    /* #RF shard operations should be moved out of common module
    public static RestStatusDup status(int successfulShards, int totalShards, ShardOperationFailedException... failures) {
        if (failures.length == 0) {
            if (successfulShards == 0 && totalShards > 0) {
                return RestStatusDup.SERVICE_UNAVAILABLE;
            }
            return RestStatusDup.OK;
        }
        RestStatusDup status = RestStatusDup.OK;
        if (successfulShards == 0 && totalShards > 0) {
            for (ShardOperationFailedException failure : failures) {
                RestStatusDup shardStatus = failure.status();
